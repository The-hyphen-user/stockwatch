...
const calculateSplits = async () => {
  ...
  try {
    ...
    const splitTest = (symbol) => {//tests if the stock has split

      
      
    const today = moment(moment.utc(), 'YYYY-MM-DD', true).add(0, 'days').format('YYYY-MM-DD');
    const yesterday = moment(moment.utc(), 'YYYY-MM-DD', true).subtract(1, 'days').format('YYYY-MM-DD');
    finnhubClient.stockSplits([symbol], yesterday, today, (error, data, response) => {
      if(data[0]) {
    ...
    userStocks.findAll({attributes: [[Sequelize.fn('DISTINCT', Sequelize.col('symbol')), 'symbol']]}).then( (UserStocks) => {
    // 'symbol', 'DISTINCT', { plain: false }
...
module.exports = { calculateSplits };
    ...
    //       calculateIndividualSplitIfNeeded(UserStock);
    //     });
    //   });
